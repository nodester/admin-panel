...
(function($) {
	...
	window.onpopstate = function(e) {
		if (e.state == undefined) return false;
	...
	$(document).ready(function() {
		...
		var $loader = $("#loader"),
			...
			template = {
			apps: {
				body: ["<td class='name'>{{name}}</td>", "<td class='port'>{{port}}</td>", "<td class='status'>{{running}}</td>", "<td class='actions'>{{actions}}</td>"].join(""),
				header: ["<th>name</th>", "<th>port</th>", "<th>app-status</th>", "<th>action</th>"].join("")
			},
			appdomains: {
				body: ["<td class='domain'>{{domain}}</td>", "<td class='appname'>{{appname}}</td>", "<td class='actions'>{{actions}}</td>"].join(""),
				header: ["<th>domain</th>", "<th>appname</th>", "<th>action</th>"].join(""),
				footer: '<td colspan="3"> <a href="#"> New Domain </a></td '
			}
		};
		...
		$("a[rel='main']").click(function(e) {
			...
			ajaxHelpers.main({
				curr_page: curr_page,
				template: template[curr_page.path]
			},
			function(template) {
				...
				onOpen: function() {
					...
					$modal.find(".form").submit(function(e) {
						...
						$.ajax({
							...
							type: "post",
							data: {
								appname: $("#params_appname").val().toLowerCase(),
								start: $("#params_start").val()
							},
							success: function(r) {
								if (r.status && r.status == "success") {
		...
		main: function(params, callback) {
			...
			$.ajax({
				...
				success: function(r) {
					...
					callback(["<thead><tr>", req_vars.template.header, "</tr></thead>", "<tbody>", Mustache.to_html("{{#items}}<tr>" + req_vars.template.body + "</tr>{{/items}}", {
						items: r
					}), "</tbody>","<tfoot><tr>", req_vars.template.footer, "</tr></tfoot>"].join(""));
